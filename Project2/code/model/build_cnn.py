...
def cnn():
    ...
    b_1 = bias([16], 0.01)
    layer_1 = tf.nn.relu(conv(input_image, W_1) + b_1)

    # layer 2 (conv)
    W_2 = weight([5, 5, 16, 32], (0.1) / (tf.sqrt(5 * 5 * 16.)))
    b_2 = bias([32], 0.01)
    layer_2 = tf.nn.relu(conv(layer_1, W_2) + b_2)

    # layer 3 (conv)
    W_3 = weight([5, 5, 32, 64], (0.1) / (tf.sqrt(5 * 5 * 32.)))
    b_3 = bias([64], 0.01)
    layer_3 = tf.nn.relu(conv(layer_2, W_3) + b_3)

    # layer 4 (conv)
    W_4 = weight([4*4*64, 100], (0.1) / (tf.sqrt(1024.)))
    b_4 = bias([100], 0.1)
    layer_3_flat = tf.reshape(layer_3, [-1, 4*4*64])
    layer_4 = tf.nn.relu(tf.matmul(layer_3_flat, W_4) + b_4)
